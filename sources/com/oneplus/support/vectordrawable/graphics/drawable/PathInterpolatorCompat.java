...
public class PathInterpolatorCompat implements Interpolator {
    ...
    /* renamed from: mX */
    private float[] f119mX;

    /* renamed from: mY */
    private float[] f120mY;
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f119mX = new float[min];
            this.f120mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, null);
                this.f119mX[i2] = fArr[0];
                this.f120mY[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f119mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f120mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f119mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f120mY[i3] - 1.0f)) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i4 = 0;
                    while (i < min) {
                        float[] fArr2 = this.f119mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            sb2.append(this.f119mX[0]);
            String str = ",";
            sb2.append(str);
            sb2.append(this.f120mY[0]);
            sb2.append(" end:");
            int i6 = min - 1;
            sb2.append(this.f119mX[i6]);
            sb2.append(str);
            sb2.append(this.f120mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f119mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f119mX[i2]) {
                length = i2;
        ...
        float[] fArr = this.f119mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f120mY[i];
        }
        float f3 = (f - fArr[i]) / f2;
        float[] fArr2 = this.f120mY;
        float f4 = fArr2[i];
