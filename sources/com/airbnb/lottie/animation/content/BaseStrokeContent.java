...
public abstract class BaseStrokeContent implements AnimationListener, KeyPathElementContent, DrawingContent {
    ...
    /* renamed from: pm */
    private final PathMeasure f46pm = new PathMeasure();
    ...
    private void applyTrimPath(Canvas canvas, PathGroup pathGroup, Matrix matrix) {
        ...
        this.f46pm.setPath(this.path, false);
        float length = this.f46pm.getLength();
        while (this.f46pm.nextContour()) {
            length += this.f46pm.getLength();
        }
        ...
        for (int size2 = pathGroup.paths.size() - 1; size2 >= 0; size2--) {
            ...
            this.f46pm.setPath(this.trimPathPath, false);
            float length2 = this.f46pm.getLength();
            float f2 = 1.0f;
