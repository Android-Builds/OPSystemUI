...
public class BubbleController implements ConfigurationListener {
    /* access modifiers changed from: private */
    public IStatusBarService mBarService;
    /* access modifiers changed from: private */
    public BubbleData mBubbleData;
    private final Listener mBubbleDataListener;
    /* access modifiers changed from: private */
    ...
    private class BubblesImeListener extends Stub {
            ...
            if (BubbleController.this.mStackView != null && BubbleController.this.mStackView.getBubbleCount() > 0) {
                BubbleController.this.mStackView.post(new Runnable(z, i) {
                    ...
                    public final void run() {
                        BubblesImeListener.this.mo11988xb7519a2f(this.f$1, this.f$2);
                    }
                });
            }
        }
        ...
        public /* synthetic */ void mo11988xb7519a2f(boolean z, int i) {
    ...
    public BubbleController(Context context, StatusBarWindowController statusBarWindowController, BubbleData bubbleData, SurfaceSynchronizer surfaceSynchronizer, ConfigurationController configurationController, NotificationInterruptionStateProvider notificationInterruptionStateProvider, ZenModeController zenModeController) {
        ...
        this.mBubbleDataListener = new Listener() {
            public void applyUpdate(Update update) {
                ...
                if (BubbleController.this.mStackView != null) {
                    ...
                    for (Pair pair : update.removedBubbles) {
                        ...
                        if (BubbleController.this.mBubbleData.hasBubbleWithKey(bubble.getKey()) || bubble.entry.showInShadeWhenBubble()) {
                            Notification notification = bubble.entry.notification.getNotification();
        ...
        this.mZenModeController.addCallback(new Callback() {
    ...
    public void ensureStackViewCreated() {
        if (this.mStackView == null) {
            ...
            statusBarView.addView(this.mStackView, statusBarView.indexOfChild(statusBarView.findViewById(R$id.scrim_behind)) + 1, new LayoutParams(-1, -1));
    ...
    public void setExpandListener(BubbleExpandListener bubbleExpandListener) {
        this.mExpandListener = new BubbleExpandListener(bubbleExpandListener) {
            private final /* synthetic */ BubbleExpandListener f$1;
    ...
    public boolean shouldAutoBubbleForFlags(Context context, NotificationEntry notificationEntry) {
        ...
        boolean z3 = shouldAutoBubbleMessages(context);
        boolean z4 = shouldAutoBubbleOngoing(context);
        boolean z5 = shouldAutoBubbleAll(context);
        if (statusBarNotification.getNotification().actions != null) {
            Action[] actionArr = statusBarNotification.getNotification().actions;
            ...
            boolean z6 = !statusBarNotification.getNotification().hasMediaSession() || (!"call".equals(statusBarNotification.getNotification().category) && statusBarNotification.isOngoing());
            Class notificationStyle = statusBarNotification.getNotification().getNotificationStyle();
            boolean equals = "msg".equals(statusBarNotification.getNotification().category);
            boolean equals2 = MessagingStyle.class.equals(notificationStyle);
            if ((((equals && z) || equals2) && z3) || ((z6 && z4) || z5)) {
                z2 = true;
            }
            return z2;
        }
        ...
        boolean equals3 = "msg".equals(statusBarNotification.getNotification().category);
        boolean equals22 = MessagingStyle.class.equals(notificationStyle2);
        z2 = true;
        return z2;
    }
    ...
    public static boolean isForegroundApp(Context context, String str) {
        List runningTasks = ((ActivityManager) context.getSystemService(ActivityManager.class)).getRunningTasks(1);
    ...
    private static boolean shouldAutoBubbleMessages(Context context) {
        return Secure.getInt(context.getContentResolver(), "experiment_autobubble_messaging", 0) != 0;
    }

    private static boolean shouldAutoBubbleOngoing(Context context) {
        return Secure.getInt(context.getContentResolver(), "experiment_autobubble_ongoing", 0) != 0;
    }

    private static boolean shouldAutoBubbleAll(Context context) {
        return Secure.getInt(context.getContentResolver(), "experiment_autobubble_all", 0) != 0;
    }

    /* access modifiers changed from: private */
    public static boolean areBubblesEnabled(Context context) {
        return Secure.getInt(context.getContentResolver(), "experiment_enable_bubbles", 1) != 0;
    }
    ...
    static boolean canLaunchInActivityView(Context context, NotificationEntry notificationEntry) {
        ...
        String str = "BubbleController";
        if (intent == null) {
            Log.w(str, "Unable to create bubble -- no intent");
            return false;
        }
        ActivityInfo resolveActivityInfo = intent.getIntent().resolveActivityInfo(context.getPackageManager(), 0);
        if (resolveActivityInfo == null) {
            StringBuilder sb = new StringBuilder();
            sb.append("Unable to send as bubble -- couldn't find activity info for intent: ");
            sb.append(intent);
            Log.w(str, sb.toString());
            return false;
        } else if (!ActivityInfo.isResizeableMode(resolveActivityInfo.resizeMode)) {
            StringBuilder sb2 = new StringBuilder();
            sb2.append("Unable to send as bubble -- activity is not resizable for intent: ");
            sb2.append(intent);
            Log.w(str, sb2.toString());
            return false;
        } else if (resolveActivityInfo.documentLaunchMode != 2) {
            StringBuilder sb3 = new StringBuilder();
            sb3.append("Unable to send as bubble -- activity is not documentLaunchMode=always for intent: ");
            sb3.append(intent);
            ...
            sb4.append("Unable to send as bubble -- activity is not embeddable for intent: ");
            sb4.append(intent);
