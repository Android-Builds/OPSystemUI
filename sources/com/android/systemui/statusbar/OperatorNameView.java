...
public class OperatorNameView extends TextView implements DemoMode, DarkReceiver, SignalCallback, Tunable {
    ...
    public void updateText() {
        ...
        while (true) {
            ...
            int subscriptionId = ((SubscriptionInfo) subscriptionInfo.get(i)).getSubscriptionId();
            State simState = this.mKeyguardUpdateMonitor.getSimState(subscriptionId);
            charSequence = ((SubscriptionInfo) subscriptionInfo.get(i)).getCarrierName();
            if (!TextUtils.isEmpty(charSequence) && simState == State.READY) {
