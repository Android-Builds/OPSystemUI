...
public abstract class AlertingNotificationManager implements NotificationLifetimeExtender {
    ...
    protected class AlertEntry implements Comparable<AlertEntry> {
        ...
        public void remove() {
            if (this.mRemoveAlertRunnable != null) {
                removeAutoRemovalCallbacks();
                AlertingNotificationManager.this.mHandler.post(this.mRemoveAlertRunnable);
            }
        }
    }
    ...
    public Stream<NotificationEntry> getAllEntries() {
        return this.mAlertEntries.values().stream().map(C1140x1f55dbd7.INSTANCE);
    }
    ...
    public void setShouldManageLifetime(NotificationEntry notificationEntry, boolean z) {
        if (z) {
            ...
            ((AlertEntry) this.mAlertEntries.get(notificationEntry.key)).removeAsSoonAsPossible();
            return;
        }
        this.mExtendedLifetimeAlertEntries.remove(notificationEntry);
    }
    ...
    /* access modifiers changed from: protected */
    public ArrayMap<String, AlertEntry> getHeadsUpEntries() {
        return this.mAlertEntries;
    }
}
