...
public class NotificationListController {
    ...
    public void tagForeground(StatusBarNotification statusBarNotification) {
        ArraySet appOps = this.mForegroundServiceController.getAppOps(statusBarNotification.getUserId(), statusBarNotification.getPackageName());
        if (appOps != null) {
            int size = appOps.size();
            for (int i = 0; i < size; i++) {
                updateNotificationsForAppOp(((Integer) appOps.valueAt(i)).intValue(), statusBarNotification.getUid(), statusBarNotification.getPackageName(), true);
            }
        }
    }
