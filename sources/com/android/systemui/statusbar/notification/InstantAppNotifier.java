...
public class InstantAppNotifier extends SystemUI implements Callbacks, Callback {
        ...
        public void onUserSwitchComplete(int i) throws RemoteException {
            InstantAppNotifier.this.mHandler.post(new Runnable() {
                public final void run() {
                    C12171.this.lambda$onUserSwitchComplete$0$InstantAppNotifier$1();
                }
            });
        }
    ...
    private void postInstantAppNotif(String str, int i, ApplicationInfo applicationInfo, NotificationManager notificationManager, int i2) {
        ...
        Action build = new Action.Builder(null, this.mContext.getString(R$string.app_info), PendingIntent.getActivityAsUser(this.mContext, 0, new Intent("android.settings.APPLICATION_DETAILS_SETTINGS").setData(Uri.fromParts("package", str3, null)), 0, null, of)).build();
        ...
        if (taskIntent == null || !taskIntent.isWebIntent()) {
            ...
            Action build2 = new Action.Builder(null, this.mContext.getString(R$string.go_to_web), PendingIntent.getActivityAsUser(this.mContext, 0, addCategory.addCategory(sb.toString()).putExtra("android.intent.extra.PACKAGE_NAME", applicationInfo2.packageName).putExtra(str5, applicationInfo2.versionCode & Integer.MAX_VALUE).putExtra(str6, applicationInfo2.longVersionCode).putExtra("android.intent.extra.INSTANT_APP_FAILURE", activityAsUser), 0, null, of)).build();
            builder = builder3;
            builder.addAction(build2);
        }
        Builder color = builder.addExtras(bundle).addAction(action).setContentIntent(pendingIntent2).setColor(this.mContext.getColor(R$color.instant_apps_color));
        Context context3 = this.mContext;
        notificationManager.notifyAsUser(str3, 7, color.setContentTitle(context3.getString(R$string.instant_apps_title, new Object[]{applicationInfo2.loadLabel(context3.getPackageManager())})).setLargeIcon(Icon.createWithResource(str3, applicationInfo2.icon)).setSmallIcon(Icon.createWithResource(this.mContext.getPackageName(), R$drawable.instant_icon)).setContentText(string2).setStyle(new BigTextStyle().bigText(string2)).setOngoing(true).build(), new UserHandle(i5));
    }
