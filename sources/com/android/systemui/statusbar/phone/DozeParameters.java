...
public class DozeParameters implements Tunable {
    ...
    public static class IntInOutMatcher {
        ...
        public IntInOutMatcher(String str) {
            ...
            if (!TextUtils.isEmpty(str)) {
                ...
                while (i < length) {
                    String str3 = split[i];
                    String str4 = "Illegal spec, must not have zero-length items: `";
                    String str5 = "`";
                    if (str3.length() != 0) {
                        ...
                        if (str3.length() != 0) {
                            if (!"*".equals(str2)) {
                                ...
                                if (this.mIsIn.indexOfKey(parseInt) < 0) {
                                    ...
                                    sb.append("Illegal spec, `");
                                    sb.append(parseInt);
                                    sb.append("` must not appear multiple times in `");
                                    sb.append(str);
                                    sb.append(str5);
                                ...
                                sb2.append("Illegal spec, `*` must not appear multiple times in `");
                                sb2.append(str);
                                sb2.append(str5);
    ...
    public float getScreenBrightnessDoze() {
        return ((float) this.mContext.getResources().getInteger(17694893)) / 255.0f;
    }
    ...
    public int getPulseVisibleDuration(int i) {
        ...
        if (i == 10) {
            return getInt("op.doze.three.key.pusle.duration.visible", R$integer.op_doze_three_key_pulse_duration_visible);
        } else if (i == 3) {
            return getInt("op.doze.pick.up.pusle.duration.visible", R$integer.op_doze_pick_up_pulse_duration_visible);
        } else if (i == 12) {
            return getInt("op.doze.single.tap.pusle.duration.visible", R$integer.op_doze_single_tap_pulse_duration_visible);
        } else if (i == 13) {
            return getInt("op.doze.fingerprint.poke.pusle.duration.visible", R$integer.op_doze_fingerprint_poke_pulse_duration_visible);
        } else {
            return getInt("doze.pulse.duration.visible", R$integer.op_doze_pulse_duration_visible);
        }
    }
    ...
    public boolean getDisplayNeedsBlanking() {
        return FORCE_BLANKING || (!FORCE_NO_BLANKING && this.mContext.getResources().getBoolean(17891410));
    }
