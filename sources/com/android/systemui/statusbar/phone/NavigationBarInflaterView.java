...
public class NavigationBarInflaterView extends FrameLayout implements ModeChangedListener {
    ...
    private void inflateChildren() {
        ...
        int i2;
        removeAllViews();
        this.mHorizontal = (FrameLayout) this.mLayoutInflater.inflate(R$layout.navigation_layout, this, false);
        if (this.mIsCustomNavBar) {
            Resources resources = this.mContext.getResources();
            if (QuickStepContract.isGesturalMode(this.mNavBarMode)) {
                i2 = R$dimen.op_gesture_navigation_side_padding;
            } else {
                i2 = R$dimen.op_rounded_corner_content_padding;
            }
            i = resources.getDimensionPixelSize(i2);
        } else {
            i = 0;
        }
        ((MarginLayoutParams) this.mHorizontal.getLayoutParams()).setMarginsRelative(i, 0, i, 0);
        addView(this.mHorizontal);
        this.mVertical = (FrameLayout) this.mLayoutInflater.inflate(R$layout.navigation_layout_vertical, this, false);
        ((MarginLayoutParams) this.mVertical.getLayoutParams()).setMarginsRelative(0, i, 0, i);
    ...
    public void inflateLayout(String str) {
        ...
        String str2 = "NavBarInflater";
        if (Build.DEBUG_ONEPLUS) {
            StringBuilder sb = new StringBuilder();
            sb.append("inflateLayout newLayout: ");
            sb.append(str);
            Log.d(str2, sb.toString());
        }
        String str3 = ";";
        String[] split = str.split(str3, 3);
        if (split.length != 3) {
            Log.d(str2, "Invalid layout.");
            split = getDefaultLayout().split(str3, 3);
        }
        String str4 = ",";
        String[] split2 = split[0].split(str4);
        String[] split3 = split[1].split(str4);
        String[] split4 = split[2].split(str4);
    ...
    private View createView(String str, ViewGroup viewGroup, LayoutInflater layoutInflater) {
        ...
        if ("home".equals(extractButton)) {
            if (this.mIsCustomNavBar) {
                return layoutInflater.inflate(R$layout.home2, viewGroup, false);
            }
            return layoutInflater.inflate(R$layout.home, viewGroup, false);
        } else if ("back".equals(extractButton)) {
            if (this.mIsCustomNavBar) {
                return layoutInflater.inflate(R$layout.back2, viewGroup, false);
            }
            return layoutInflater.inflate(R$layout.back, viewGroup, false);
        } else if ("recent".equals(extractButton)) {
            if (this.mIsCustomNavBar) {
                return layoutInflater.inflate(R$layout.recent_apps2, viewGroup, false);
            }
            return layoutInflater.inflate(R$layout.recent_apps, viewGroup, false);
        } else if (str2.equals(extractButton)) {
            if (this.mIsCustomNavBar) {
                return layoutInflater.inflate(R$layout.menu_ime2, viewGroup, false);
            }
            return layoutInflater.inflate(R$layout.menu_ime, viewGroup, false);
        } else if (str3.equals(extractButton)) {
            View inflate = layoutInflater.inflate(R$layout.nav_key_space, viewGroup, false);
            if (this.mIsCustomNavBar) {
                inflate.getLayoutParams().width = (int) this.mContext.getResources().getDimension(R$dimen.op_navigation_side_padding);
            }
            return inflate;
            ...
            if ("nav".equals(extractButton)) {
                return layoutInflater.inflate(R$layout.nav, viewGroup, false);
            }
            ...
            KeyButtonView keyButtonView = (KeyButtonView) inflate2;
    ...
    private void updateChildrenMargin() {
        ...
        MarginLayoutParams marginLayoutParams = (MarginLayoutParams) this.mHorizontal.getLayoutParams();
        marginLayoutParams.setMarginsRelative(i, 0, i, 0);
        updateViewLayout(this.mHorizontal, marginLayoutParams);
        MarginLayoutParams marginLayoutParams2 = (MarginLayoutParams) this.mVertical.getLayoutParams();
