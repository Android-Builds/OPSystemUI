...
public class EmergencyCryptkeeperText extends TextView {
    ...
    public void update() {
        ...
        boolean z = Global.getInt(this.mContext.getContentResolver(), "airplane_mode_on", 0) == 1;
        if (!isNetworkSupported || z) {
            setText(null);
            setVisibility(8);
            return;
        }
        List subscriptionInfo = this.mKeyguardUpdateMonitor.getSubscriptionInfo(false);
        int size = subscriptionInfo.size();
        boolean z2 = true;
        CharSequence charSequence = null;
        for (int i2 = 0; i2 < size; i2++) {
            State simState = this.mKeyguardUpdateMonitor.getSimState(((SubscriptionInfo) subscriptionInfo.get(i2)).getSubscriptionId());
            CharSequence carrierName = ((SubscriptionInfo) subscriptionInfo.get(i2)).getCarrierName();
        ...
        if (z2) {
            if (size != 0) {
                charSequence = ((SubscriptionInfo) subscriptionInfo.get(0)).getCarrierName();
            } else {
                charSequence = getContext().getText(17039911);
